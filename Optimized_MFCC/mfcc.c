...
*/
int fix_fft(short fr1[], short fi1[], short m, short inverse)
{
...
void create_mel_fbank(mfcc_t *mfcc) {
    ...
    for (int i=0; i<mfcc->num_fbank+2; i++)
    {
        ...
        mfcc->mel_fbins_fix[i] = (int)(mfcc->mel_fbins[i] * 32767.0f);
    }
...
void  mfcc_compute(mfcc_t *mfcc, const int16_t * audio_data, float* mfcc_out)
{
   ...
   // FFT data structure
    ...
    for (int i = 0; i <= mfcc->frame_len_padded/2; i++) {
        mfcc->frame[i] = (data_re2[i] * data_re2[i] + data_im2[i]* data_im2[i]);
    }
    for (int i = 0; i <= mfcc->frame_len_padded/2; i++) {
        mfcc->frame[i] /= 32767.0f;
    }
...
void apply_filter_banks(mfcc_t *mfcc){
    ...
    for (int j = 0; j < mfcc->num_fbank ; j++){
        ...
        for (int i = left + 1; i < center; i++) {
            float mel_factor = (i - left) / (center - left);
            mel_energy += mfcc->frame[i] * mel_factor;
        }
        for (int i = center; i < right; i++) {
            float mel_factor = (right - i) / (right - center);
            mel_energy += mfcc->frame[i] * mel_factor;
        }
        if (mel_energy == 0.0f)
            mel_energy = FLT_MIN;
