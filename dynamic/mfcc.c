...
mfcc_t *mfcc_create(int num_mfcc_features, int feature_offset, int num_fbank, int frame_len, float preempha, int is_append_energy)
{
    ...
    // MARK: - this can be static
    // the frame_len_padded is related to the static frame_len 512
    // it can be = 512, because it is already a power of 2
	// Round-up to nearest power of 2.
	mfcc->frame_len_padded = (int)powf(2, ceilf((logf(frame_len)/logf(2))));

    // MARK: - this can be static
    // because we already have the mfcc->frame_len_padded
    ...
    // MARK: - this can be static
    // num_fbank is staticaly defined as 26
    // because we already have the mfcc->frame_len_padded
	mfcc->mel_energies = mfcc_malloc(sizeof(float)*mfcc->num_fbank );

    
    // MARK: - this can be static
    // frame_len is staticaly defined as 512
    ...
    // MARK: - this can be static // inside function
...
#else
    // MARK: - this can be static // sizeof(float) * 512 * 2
...
float ** create_mel_fbank(mfcc_t *mfcc) {
    ...
    float * bin =  mfcc_malloc(sizeof(float) * (mfcc->num_fbank+2));                // MARK: - this can be static // sizeof(float) * (26+2)
    ...
    // MARK: - this can be static // sizeof(float*) * (26) [i][?]
    float ** mel_fbank =  mfcc_malloc(sizeof(float*) * mfcc->num_fbank);

    
    // MARK: - this can be static // sizeof(float*) * (512/2+1) [?][j]
    ...
    // MARK: - this should be removed or replaced
    mfcc_free(bin);
    return mel_fbank;
